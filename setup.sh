...
#
# fish is a prerequisite (this repo manages fish config but does not install the
# shell): install it with your package manager first, e.g. `sudo apt install fish`.
set -euo pipefail
...
# fish must already be installed — the managed config is useless without it.
if ! command -v fish >/dev/null 2>&1; then
  err "fish is not installed — install it first (e.g. sudo apt install fish, brew install fish)"
  exit 1
fi
